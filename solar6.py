...
solar["last_changed"] = pd.to_datetime(
    solar["last_changed"]).dt.tz_localize(None)
...
# Feature engineering
solar["hour"] = solar["last_changed"].dt.hour + \
    solar["last_changed"].dt.minute/60
...
future_times = pd.date_range(
    last_t + timedelta(hours=1), periods=24, freq="H").tz_localize(None)
...
for i in range(24):
    ...
    for lag in range(1, 25):
        row[f"lag_{lag}"] = prev_vals[lag-1] if lag - \
            1 < len(prev_vals) else pred_base[i-(lag-1)]
    # scale back to watts
    pred_base.append(96 * model.predict(pd.DataFrame([row]))[0])
...
first_plotted = hist["last_changed"].min(
).normalize()           # e.g. 2025-04-22
...
plt.plot(pred_times, pred_df["predicted_solar_output"],
         "--", label="Forecast (24 h)")
plt.xlabel("Time")
